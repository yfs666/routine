...
public class MappingControlGem implements Gem {
    ...
    private MappingControlGem( BuilderImpl builder ) {
    ...
    * accessor
    *
    * @return the {@link GemValue} for {@link MappingControlGem#value}
    */
    ...
    public static MappingControlGem instanceOn(AnnotationMirror mirror ) {
        return build( mirror, new BuilderImpl() );
    }

    public static  <T> T  build(Element element, Builder<T> builder) {
        AnnotationMirror mirror = element.getAnnotationMirrors().stream()
            .filter( a ->  "org.mapstruct.control.MappingControl".contentEquals( ( ( TypeElement )a.getAnnotationType().asElement() ).getQualifiedName() ) )
            .findAny()
            .orElse( null );
    ...
    public static <T> T build(AnnotationMirror mirror, Builder<T> builder ) {
        ...
        for ( String methodName : defaultValues.keySet() ) {
            ...
            if ( "value".equals( methodName ) ) {
                builder.setValue( GemValue.createEnum( values.get( methodName ), defaultValues.get( methodName ) ) );
            }
        }
    ...
    public interface Builder<T> {
       ...
       /**
        * Sets the {@link GemValue} for {@link MappingControlGem#value}
        *
        * @return the {@link Builder} for this gem, representing {@link MappingControlGem}
        */
