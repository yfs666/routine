...
public class MappingsGem implements Gem {
    ...
    * accessor
    *
    * @return the {@link GemValue} for {@link MappingsGem#value}
    */
    ...
    public static  <T> T  build(Element element, Builder<T> builder) {
        AnnotationMirror mirror = element.getAnnotationMirrors().stream()
            .filter( a ->  "org.mapstruct.Mappings".contentEquals( ( ( TypeElement )a.getAnnotationType().asElement() ).getQualifiedName() ) )
            .findAny()
            .orElse( null );
    ...
    public static <T> T build(AnnotationMirror mirror, Builder<T> builder ) {
        ...
        for ( String methodName : defaultValues.keySet() ) {
            ...
            if ( "value".equals( methodName ) ) {
                builder.setValue( GemValue.createArray( values.get( methodName ), defaultValues.get( methodName ), MappingGem::instanceOn ) );
            }
        }
    ...
    public interface Builder<T> {
       ...
       /**
        * Sets the {@link GemValue} for {@link MappingsGem#value}
        *
        * @return the {@link Builder} for this gem, representing {@link MappingsGem}
        */
        Builder setValue(GemValue<List<MappingGem>> methodName );
